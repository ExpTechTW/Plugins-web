...
function PluginCardReleaseButton({
  pluginId,
  release,
  variant = "filled",
}: PluginCardReleaseButtonProps) {
...
function SmallStats({
  ...
  const clazz: string =
    "flex flex-row gap-0.5 items-center text-sm cursor-default";
  ...
  if (fullToolTip) {
    ...
    return (
      <div className={clazz}>
        <ClickableTooltip label={tooltip}>{icon}</ClickableTooltip>
        {text}
      </div>
    );
  }
...
export function PluginCard({ plugin }: { plugin: SimplePlugin }) {
  ...
  const repositoryButton = (
    <ActionIcon
      color="gray.7"
      aria-label={`Visit GitHub repository for ${plugin.id}`}
    >
      <Link
        href={plugin.repos}
        aria-label={`GitHub repository for ${plugin.id}`}
      >
        <GithubIcon />
      </Link>
    </ActionIcon>
  ...
  const lastUpdateText =
    plugin.last_update_time !== undefined ? (
      ...
      <div className="grow w-full">
        <div className="col-span-5 flex flex-col justify-between">
          <div className="mb-3 ml-1 min-h-[25px]">
            <PluginCardDescription description={plugin.description} />
          </div>
            ...
            <div className="flex flex-wrap gap-x-5 gap-y-2">
              <div className="flex flex-row gap-3 self-end">
                ...
                <SmallStats
                  ...
                  text={<p>{plugin.downloads}</p>}
                  tooltip={t("total_downloads")}
                  fullToolTip={true}
                />
              </div>
